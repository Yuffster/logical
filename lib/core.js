...
(function(scope) {
	...
	var sandboxEval, helpers = {}, templates = {};
	...
	function addTemplate(name, content) {
		templates[name] = compile(content);
	}
	...
	if (typeof window === "undefined") {
		sandboxEval = function(code, locals, callback) {
		 ...
		 */
		sandboxEval = function(code, locals, cb) {
	...
	function render(code, data, cb) {
		...
		if (templates[code]) return templates[code].render(data);
		...
		return sandboxEval("var _o_='';"+code+";_o_;", data, cb);

...
(function(scope) {
	...
	function render(code, data, helpers) {

		data = data || {};

		if (templates[code]) {
			return templates[code].render(data);
		}
		...
		for (var k in helpers) context[k] = helpers[k];

		return sandboxEval("var _o_='';"+code+";_o_;", data, helpers);
	...
	function compile(code) {
		...
		if (!code.match(tagMatch)) {
			code = "_o_+="+JSON.stringify(code)+";";
		} else {
			var c = 0;
	...
	Instance.prototype.render = function(code, data) {
		var context = {};
		for (var k in helpers)      context[k] = helpers[k];
		for (var k in this.helpers) context[k] = this.helpers[k];
		return (this.templates[code] || compile(code)).render(data, context);	
	};
	...
	api.instance = function() { return new Instance(); };

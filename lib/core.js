...
(function(scope) {
	...
	if (typeof window === "undefined") {
		...
		sandboxEval = function(code, locals, helpers) {
			...
			// Inject global and local helpers.
			for (k in helpers) context[k] = helpers[k];
			if (this.logicalInstance) {
				for (k in this.helpers) context[k] = helpers[k];
			}
		...
		sandboxEval = function(code, locals, helpers) {
			...
			var iframe = document.createElement('iframe'), iwin, k, output;
			document.body.appendChild(iframe);
			iwin = window.frames[frames.length-1];

			// Enable global and local helpers

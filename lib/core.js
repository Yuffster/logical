...
(function(scope) {
	...
	function render(code, data) {
		...
		// Merge global helpers with local helpers.
		if (this.logicalInstance) {
			context = this.helpers;
	...
	Instance.prototype.render = function(code, data) {
		return (this.templates[code] || compile(code)).render(data);	
	};
	...
	api.render = function(code, data, cb) {
		return (templates[code] || compile(code)).render(data, cb);
	};
